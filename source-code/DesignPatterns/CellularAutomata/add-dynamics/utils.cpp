...
CAOptions parse_arguments(int argc, char* argv[]) {
    ...
    desc.add_options()
        ...
        ("dynamics", po::value<std::string>()->default_value("collective"), "dynamics: 'collective' or 'individual'")
    ...
    try {
        ...
        options.dynamics = vm["dynamics"].as<std::string>();
        if (options.dynamics != "collective" && options.dynamics != "individual") {
            throw std::runtime_error("dynamics must be 'collective' or 'individual'");
        }
...
std::unique_ptr<Dynamics> create_dynamics(const CAOptions& options) {
    ...
    } else if (options.dynamics == "collective") {
        std::make_unique<CollectiveDynamics>(rule, options.nr_cells);
    } else {
        throw std::invalid_argument("Unknown dynamics: " + options.dynamics);
    }
