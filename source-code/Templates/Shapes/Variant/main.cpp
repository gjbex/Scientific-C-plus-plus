...
double total_area(const Data& data) {
    ...
    for (const auto& shape : data) {
        std::visit([&total](auto&& arg) { total += arg.area(); }, shape);
    }
    return total;
}
...
Data create_shapes(std::size_t nr_shapes) {
    ...
    for (std::size_t i = 0; i < nr_shapes; ++i) {
        switch (shape_dist(gen)) {
            case 0:
                data.push_back(Circle(size_dist(gen)));
                break;
            case 1:
                data.push_back(Square(size_dist(gen)));
                break;
            case 2:
                data.push_back(Triangle(size_dist(gen), size_dist(gen)));
                break;
        }
    }
    return data;
